...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const updated = await prisma.courseType.update({ where: { id: params.id }, data })
    ...
    return NextResponse.json({ success: false, error: 'Failed to update course type' }, { status: 500 })
  }
}
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    await ensurePrismaConnected()
    const updated = await prisma.courseType.update({ where: { id: params.id }, data: { isActive: false } })
    return NextResponse.json({ success: true, data: updated })
  } catch (e: any) {
    if (e?.code === 'P2025') return NextResponse.json({ success: false, error: 'Not found' }, { status: 404 })
    return NextResponse.json({ success: false, error: 'Failed to delete course type' }, { status: 500 })
  }
}
